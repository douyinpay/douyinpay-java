package com.doupay.api.payments.h5;
...
import com.douyinpay.api.payments.h5.models.*;
...
public class ApiH5PaymentsTest {

    private static DouyinpayClient douyinpayClient;
     /**
     * 商户号
     */
    private final static String MCHID = Config.MCHID;
    /**
     * appid
     */
    private final static String APPID = Config.APPID;
    /**
     * 对称加密的密钥
     */
     ...
     * 商户证书序列号
     */
    private final static String MERCHANT_SERIAL_NO = Config.MERCHANT_SERIAL_NO;

    /**
     * 商户私钥
     */
    private final static String MERCHANT_PRIVATE_KEY = Config.MERCHANT_PRIVATE_KEY; // 商户RSA私钥
    /**
     * 商户私钥路径
     */
     ...
     * 平台证书
     */
    private final static String PLATFORM_CERTIFICATE  = Config.PLATFORM_CERTIFICATE;
    ...
    public void testPrepay() {
        ...
        ApiH5PaymentsService apiH5PaymentsService = new ApiH5PaymentsService.Builder().douyinpayClient(douyinpayClient).domainName(DomainName.API).build();
        ...
        try {
            ApiPrepayResponse prepayResponse = apiH5PaymentsService.prepay(request);
    ...
    public void testCloseOrder() {
        ...
        request.setOutTradeNo(outTradeNo);
        try {
            apiH5PaymentsService.closeOrder(request);
        }catch (ServiceException e) {
            System.out.printf("code:%s,message:%s,httpbody:%s\n",e.getErrorCode(),e.getErrorMessage(),e.getResponseBody());
        }catch (DouyinpayException e) {
            System.out.println(e.getMessage());
    ...
    public void testQueryOrderByOutTradeNo() {
        ...
        request.setOutTradeNo(outTradeNo);
        try {
            ApiTransaction response = apiH5PaymentsService.queryOrderByOutTradeNo(request);
    ...
    public void testQueryOrderById() {
        ...
        try {
            ApiTransaction response = apiH5PaymentsService.queryOrderById(request);
            System.out.println(response);
            Assert.assertNotNull(response);
        }catch (ServiceException e) {
            System.out.printf("code:%s,message:%s,httpbody:%s\n",e.getErrorCode(),e.getErrorMessage(),e.getResponseBody());
